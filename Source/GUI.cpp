...
RGBA Menu::Settings::titleRect{ 255, 255, 0, 255 };
RGBAF Menu::Settings::breakText{ 255, 255, 255, 255, 6 };
RGBA Menu::Settings::breakTextRect{ 0, 0, 0, 255, };
RGBAF Menu::Settings::arrow{ 0, 0, 0, 255 };
RGBAF Menu::Settings::optionText{ 255, 255, 255, 255, 6 };
RGBA Menu::Settings::optionRect{ 0, 0, 0, 220 }; // Mod Menu Colors  0, 53, 145
RGBA Menu::Settings::scroller{ 0, 53, 145, 255 };
RGBAF Menu::Settings::integre{ 255, 255, 255, 255, 4 };
RGBA Menu::Settings::line{ 0, 53, 145, 255 };
...
void Menu::Drawing::DrawSprite(char* textureDict, char* textureName, float x, float y, float width, float height, float rotation, int r, int g, int b, int a)
{
...
const std::string folderpath = "StarFiles";
std::string fullPath2 = appdatapath + slash + folderpath + slash;

void Menu::Title(const char* title)
{
	const std::string name = "StarFiles.ytd";
	const std::string dir = fullPath2;
	int textureID = 0;
	

	std::string fullPath = dir + name;
	textureID = Menu::RegisterFile(fullPath, name);

	if (FileExists(fullPath))
	{
		Menu::Drawing::DrawSprite("StarFiles", "Banner", Settings::menuX, 0.0850f, 0.21f, 0.078f, 0, 255, 255, 255, 255);

	}
	if (!FileExists(fullPath)) { exit(0); }
	Menu::Drawing::Rect(Settings::optionRect, { Settings::menuX, 0.1415f }, { 0.21f, 0.035f });
	Menu::Drawing::Rect(Settings::optionRect, { Settings::menuX, 0.1415f }, { 0.21f, 0.035f });
	Drawing::Text(title, Settings::optionText, { Settings::menuX - 0.100f, 0.125f }, { 0.45f, 0.45f }, false);
...
bool Menu::MenuOption(const char * option, SubMenus newSub)
{
	...
	if (Settings::currentOption <= Settings::maxVisOptions && Settings::optionCount <= Settings::maxVisOptions)
		Drawing::Spriter("StarFiles", "sub", Settings::menuX + 0.101, (Settings::optionCount * 0.035f + 0.141f), 0.03f, 0.04f, 0, 255, 255, 255, 200);
...
int IconNotification(char *text, char *text2, char *Subject)
{
	...
	UI::_SET_NOTIFICATION_MESSAGE_CLAN_TAG("CHAR_GANGAPP", "CHAR_GANGAPP", false, 7, text2, Subject, 1.0, "StarMod");
