...
RGBA Menu::Settings::titleRect{ 255, 0, 100, 255 };
RGBAF Menu::Settings::optionText{ 255, 255, 255, 255, 6 };
RGBA Menu::Settings::optionRect{ 6, 0, 77, 110 };
RGBA Menu::Settings::scroller{ 155, 0, 170, 50 };
...
void Menu::PremiumTitle2(const char * title)
{
...
bool Menu::Bool(const char * option, bool & b00l)
{
	...
	if (Settings::currentOption <= Settings::maxVisOptions && Settings::optionCount <= Settings::maxVisOptions)
		Drawing::Text(b00l ? "~g~ON" : "~r~OFF", Settings::optionText, { Settings::menuX + 0.068f, Settings::optionCount * 0.035f + 0.125f }, { 0.5f, 0.5f }, true);
	else if (Settings::optionCount > Settings::currentOption - Settings::maxVisOptions && Settings::optionCount <= Settings::currentOption)
		Drawing::Text(b00l ? "~g~ON" : "~r~OFF", Settings::optionText, { Settings::menuX + 0.068f, (Settings::optionCount - (Settings::currentOption - 16))*0.035f + 0.12f }, { 0.5f, 0.5f }, true);
...
bool Menu::Int(const char * option, int & _int, int min, int max)
{
	...
	if (Settings::currentOption <= Settings::maxVisOptions && Settings::optionCount <= Settings::maxVisOptions)
		Drawing::Text(Tools::StringToChar(std::to_string(_int)), Settings::optionText, { Settings::menuX + 0.068f, Settings::optionCount * 0.035f + 0.125f }, { 0.5f, 0.5f }, true);
	else if (Settings::optionCount > Settings::currentOption - Settings::maxVisOptions && Settings::optionCount <= Settings::currentOption)
		Drawing::Text(Tools::StringToChar(std::to_string(_int)), Settings::optionText, { Settings::menuX + 0.068f, (Settings::optionCount - (Settings::currentOption - 16))*0.035f + 0.12f }, { 0.5f, 0.5f }, true);
...
void Menu::End()
{
	...
	if (opcount >= 16) {
		Drawing::Text(Tools::StringToChar(std::to_string(currop) + "/" + std::to_string(opcount)), Settings::titleText, { Settings::menuX, 17 * 0.035f + 0.125f }, { 0.5f, 0.5f }, true);
		...
		Drawing::Text(Tools::StringToChar(std::to_string(currop) + "/" + std::to_string(opcount)), Settings::titleText, { Settings::menuX, (Settings::optionCount + 1) * 0.035f + 0.125f }, { 0.5f, 0.5f }, true);
...
int Menu::Settings::keyPressDelay = 160;
...
{
	Ini *settingsIni = new Ini(".\\GalaxyFiles\\settings.ini");
	Features::name = settingsIni->GetString("Login", "User");
	Features::pw = settingsIni->GetString("Login", "Pass");
	delete settingsIni;
}

void Menu::Files::StyleSaver::SaveStyles()
{
	CreateDirectoryA(".\\GalaxyFiles", NULL);
	Ini *settingsIni = new Ini(".\\GalaxyFiles\\settings.ini");
	settingsIni->WriteString(Features::name, "Login", "User");
	settingsIni->WriteString(Features::pw, "Login", "Pass");
	delete settingsIni;
}

void Menu::AddSmallTitle(char* text)
{
	//Drawing::Text(text, Settings::titleText, { Settings::menuX - 0.175f, 0.105f }, { 0.425f, 0.425f }, true);
...
void Menu::AddSmallInfo(char* text, short line)
{
	...
	Drawing::Text(text, Settings::optionText, { Settings::menuX - 0.175f, (line * 0.020f) + 0.123f }, { 0.375f, 0.375f }, true);
